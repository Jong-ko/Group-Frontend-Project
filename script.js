...
async function fetchPlaylist(searchTerm) {
  try {
    ...
    const results = await fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${searchTerm}%20weather%20music&order=relevance&type=video&maxResults=10&videoEmbeddable=true&regionCode=us&key=AIzaSyBLu9aikLuuSZWHvqwSRuiJ1oKfjTK_ig8`
    );
  ...
  //try {
    ...
    if (citySearchJson.length > 1) {
      ...
      for (let i = 0; i < citySearchJson.length; i++) {
        let stateCode = states.get(citySearchJson[i].state);
        let cityoption = `${citySearch},${stateCode},${citySearchJson[i].country}`;
        suggestionsList.innerHTML += `
